          ...
          <tbody>
            @foreach ($records as $record)
              <tr>
                ...
                @endforeach
                <td>
                  <a href="{{ route('items.edit', $record->getKey()) }}" class="btn btn-sm btn-primary">Edit</a>
                  <form action="{{ route('items.destroy', $record->getKey()) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this item?')">
                    @csrf
                    @method('DELETE')

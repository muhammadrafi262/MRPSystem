...
<div class="pc-container">
  <div class="pc-content">
          ...
          <tbody>
            @foreach ($records as $record)
              <tr>
                ...
                @endforeach
                <td>
                  <a href="{{ route('orders.edit', $record->getKey()) }}" class="btn btn-sm btn-primary">Edit</a>
                  <form action="{{ route('orders.destroy', $record->getKey()) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this item?')">
                    @csrf
                    @method('DELETE')
    ...
    <div class="page-block">
      <a href="{{ route('orders.create') }}" class="btn btn-success">Tambah Data</a>
    </div>
  </div>
</div>
@endsection